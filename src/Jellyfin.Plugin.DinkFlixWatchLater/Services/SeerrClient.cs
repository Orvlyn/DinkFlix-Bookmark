...
public class SeerrClient{
 ...
 public async Task<bool> TestConnectionAsync(){try{using var r=await C().GetAsync("api/v1/status");return r.IsSuccessStatusCode;}catch{return false;}}
}
...
[JsonIgnore]public string Type=>(MediaType??MediaTypeSnake??"").Equals("tv",StringComparison.OrdinalIgnoreCase)?"tv":"movie";
